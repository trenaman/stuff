...
public class ConsoleMoveReporter implements MoveReporter {
  ...
  public void movedMessage(String queueName, long timestamp) {
    ...
    if (MessageHistory.isAncient(timestamp)) {
      ancientMsgs++;
    } else if (MessageHistory.isRecent(timestamp)) {
      recentMsgs++;
    } else if (MessageHistory.isOld(timestamp)) {
      oldMsgs++;
    } else if (MessageHistory.isCurrent(timestamp)) {
      currentMsgs++;
    }
  ...
  public void summarize(String journalLocation) {
    ...
    out.printf("\n\nSummary:\n\n");
    out.printf("%-40s %7d\n", "Ancient messages ( > " + MessageHistory.OLD_THRESHOLD_WEEKS + " weeks)", ancientMsgs);
    out.printf("%-40s %7d\n", "Old messages ( <= " + MessageHistory.OLD_THRESHOLD_WEEKS + " weeks)", oldMsgs);
    out.printf("%-40s %7d\n", "Recent messages ( <= " + MessageHistory.RECENT_THRESHOLD_WEEKS + " weeks)", recentMsgs);
    out.printf("%-40s %7d\n", "Current messages ( <= " + MessageHistory.CURRENT_THRESHOLD_WEEKS + " weeks)", currentMsgs);
